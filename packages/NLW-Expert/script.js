...
// new Set serve para criar coisas novas em estrutura de dados ja que set é um conjunto então da para adicionar ou tirar. 
...
for (const item of perguntas) {
  ...
  quizItem.querySelector("h3").textContent = item.pergunta // modificando o h3
    ...
    // for dentro de for é aninhamento 
  for (let resposta of item.respostas) {
    const dt = quizItem.querySelector("dl dt").cloneNode(true) // clonando as respostas e apresentando elas na tela
    dt.querySelector("span").textContent = resposta
    // nessa parte de baixo está seelecionando o input setando um atributo e pegando umobjeto que o array perguntas e terá a fução indexOf da qual será passado para pesquisar um indice
    ...
    dt.querySelector('input').onchange = (event) => {
      ...
      corretas.delete(item) // vai deletar o item conforme vc muda de opnião
      if (estaCorreta) {
          corretas.add(item)
      }
